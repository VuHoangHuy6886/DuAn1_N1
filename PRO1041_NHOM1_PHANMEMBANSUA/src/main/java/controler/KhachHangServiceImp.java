...
public class KhachHangServiceImp implements repository.KhachHangService {
    ...
    @Override
    public List<KhachHang> listKhachHangSearch(String tenKhachHang) {
        List<KhachHang> listKhachhang = new ArrayList<>();
        try (Connection conn = Database.JdbcUtil.getConnection()) {
            String sql = "SELECT * FROM KhachHang Where TenKH like ? Or Id_KH = ? ";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, "%" + tenKhachHang + "%");
            ps.setInt(2, view.KhachHangInHoaDon.idKhachHang);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                KhachHang kh = new KhachHang();
                kh.setId(rs.getInt("Id_KH"));
    ...
    public String xoaKhachHang(KhachHang kh) {
        try (Connection conn = JdbcUtil.getConnection()) {
            ...
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, view.KhachHangView.id);
...
//            }
    ...
    @Override
    public List<KhachHang> listKhachHangByID(int id) {
        List<KhachHang> listKhachhang = new ArrayList<>();
        try (Connection conn = Database.JdbcUtil.getConnection()) {
